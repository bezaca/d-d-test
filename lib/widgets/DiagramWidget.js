...
import { TextNodeWidget } from '../../src/components/nodes/textnode/TextNodeWidget';
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      windowListener: window.addEventListener('keydown', event => {
        ...
        // Delete all selected
        if ([8, 46].indexOf(event.keyCode) !== -1 && selectedItems.length && deleteItems && selectedItems[0].nodeType !== 'textnode') {
    ...
    if (action instanceof SelectingAction && multiselectDrag) {
      ...
      action.selectionModels.forEach(model => {
        ...
        if (model.model instanceof NodeModel) {
          ...
          model.model.x = model.initialX + (Math.ceil(differentialX / 30) * 30);
          model.model.y = model.initialY + (Math.ceil(differentialY / 30) * 30);

        } else {
          model.model.x = model.initialX + differentialX;
          model.model.y = model.initialY + differentialY;
        }
      ...
      action.selectionModels.forEach(model => {
        ...
        if (model.model instanceof PointModel) {
          ...
          if (element.model instanceof PortModel) {
            ...
            actionOutput.linkModel = model.model.remove();
          }
        }
      });
    }
